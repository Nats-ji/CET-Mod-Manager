...
std::vector<std::string> ScanMods::scan_mods(const std::filesystem::path aPath, const std::string aExtension, bool aCheckFolder)
{
  ...
  for (const auto& entry : std::filesystem::directory_iterator(aPath))
  {
    if (aCheckFolder && entry.is_directory())
    {
      ...
      {
        if (sub_entry.is_regular_file() && sub_entry.path().extension() == aExtension)
        {
    ...
    else if (entry.is_regular_file() && entry.path().extension() == aExtension)
      mod_list.push_back(entry.path().filename().string());
  }
  return mod_list;
}
